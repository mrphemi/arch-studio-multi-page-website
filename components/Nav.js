...
const desktopListItemVariants = {
  hidden: { opacity: 0, y: -30 },
  visible: { opacity: 1, y: 0 },
};

const pageIndicatorLineVariants = {
  hidden: { height: 0 },
  visible: { height: "112px", transition: { duration: 0.5, type: "tween" } },
};

const pageIndicatorTextVariants = {
  hidden: { opacity: 0 },
  visible: {
    opacity: 1,
...
export default function Nav() {
  ...
  const pageName = router.asPath.slice(1) || "home";

  return (
    <nav className="relative z-20 bg-white">
      {/* Page indicator */}
      <div className="absolute left-10 xl:left-16 hidden md:flex flex-col space-y-8 -z-10">
        <motion.span
          initial="hidden"
          animate="visible"
          variants={pageIndicatorLineVariants}
          className="border-l-2 border-very-light-grey"
        ></motion.span>
        <motion.span
          initial="hidden"
          animate="visible"
          variants={pageIndicatorTextVariants}
          className="transform rotate-90 origin-left uppercase text-very-light-grey tracking-[15px]"
        >
          {pageName}
        </motion.span>
      </div>

      {/* Logo and Nav - Desktop and Tablet */}
      <div className="container py-8 md:py-14 flex justify-between md:justify-start md:space-x-16 xl:space-x-24 bg-white">
        {/* Logo */}
        <Link href="/" passHref>
          <motion.svg
            initial={{ y: -100, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ delay: 0.8, duration: 0.3, type: "tween" }}
        ...
        <motion.ul
          ...
          className="hidden md:flex self-center space-x-6"
        >
          {links.map((link) => (
            <motion.li variants={desktopListItemVariants} key={link.url}>
              <Link href={link.url}>
                <a className="capitalize text-medium-grey xl:hover:text-very-dark-blue">
                  {link.text}
                </a>
              </Link>
            </motion.li>
      ...
      <motion.ul
        ...
        className="space-y-8 md:hidden bg-very-light-grey p-12 w-11/12 ml-auto absolute right-0"
      >
        {links.map((link) => (
          <motion.li
          ...
          >
            <Link href={link.url}>
              <a className="capitalize text-very-dark-blue font-bold">
                {link.text}
              </a>
            </Link>
          </motion.li>
